...
function addBoatRecord(buildings, boat) {
...
function buildHormuzStrait(seed) {
  const rand = mulberry32(seed);
  const boats = [];
  const lanes = [
    { z: -72, heading: -Math.PI * 0.5, traffic: "westbound" },
    { z: -42, heading: -Math.PI * 0.5, traffic: "westbound" },
    { z: -12, heading: Math.PI * 0.5, traffic: "eastbound" },
    { z: 18, heading: Math.PI * 0.5, traffic: "eastbound" },
    { z: 42, heading: rand() > 0.5 ? Math.PI * 0.5 : -Math.PI * 0.5, traffic: "coastal" }
  ];
  const classes = [
    { zone: "supertanker", width: 44, depth: 180, height: 24, weight: 0.16 },
    { zone: "container", width: 36, depth: 142, height: 32, weight: 0.2 },
    { zone: "cargo", width: 30, depth: 104, height: 22, weight: 0.24 },
    { zone: "patrol", width: 13, depth: 46, height: 13, weight: 0.18 },
    { zone: "fastboat", width: 7, depth: 22, height: 6, weight: 0.22 }
  ];
  ...
  lanes.forEach((lane, laneIndex) => {
    ...
    for (let index = 0; index < count; index += 1) {
      const spec = pickClass();
      const x = -410 + (index + rand() * 0.72) * (820 / count) + (rand() - 0.5) * 22;
      const z = lane.z + (rand() - 0.5) * 12;
      const lengthScale = 0.86 + rand() * 0.32;
      const beamScale = 0.88 + rand() * 0.22;
      addBoatRecord(boats, {
        id: `lane-${laneIndex}-${lane.traffic}-${index}-${spec.zone}`,
        x,
        z,
        width: spec.width * beamScale,
        depth: spec.depth * lengthScale,
        height: spec.height * (0.84 + rand() * 0.28),
        heading: lane.heading + (rand() - 0.5) * 0.16,
        zone: spec.zone
      });
    }
  });

  return boats.sort((a, b) => a.z - b.z || a.x - b.x);
}
...
function createRound(roundId) {
  ...
  const buildings = buildHormuzStrait(seed);
  return {
    scenario: SCENARIO,
    roundId,
    seed,
    citySource: SCENARIO,
    cityLayoutName: "Strait of Hormuz",
    phase: "active",
    startedAt: Date.now(),
    endedAt: null,
    cooldownEndsAt: null,
...
function currentPayload() {
  ...
  return {
    ...
    citySource: state.citySource || SCENARIO,
    cityLayoutName: state.cityLayoutName || "Strait of Hormuz",
    phase: state.phase,
...
function chooseStrikeVehicle(building) {
  ...
  if ((building.zone === "supertanker" || building.zone === "container") && Math.random() < 0.18) {
    return "airplane";
  }
  return "missile";
}
...
function markDestroyed(buildingId, playerId, playerName) {
  ...
  if (!building) {
    ...
    return { ok: false, code: 404, error: "Vessel not found." };
  }
  if (building.destroyedAt) {
    ...
    return { ok: false, code: 409, error: "Vessel already destroyed." };
  }
  if (Array.from(activeStrikes.values()).some((item) => item.buildingId === buildingId)) {
    ...
    return { ok: false, code: 409, error: "Vessel already targeted." };
  }
